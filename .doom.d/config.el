...
(defun fb/better-evil-jump-item ()
  ...
  (if mark-active
      ;; (call-interactively 'evilmi-jump-items)                 ;; → not working
      (setq unread-command-events (listify-key-sequence "%"))    ;; → working as expected, the key `%` is bound to evilmi-jump-items
