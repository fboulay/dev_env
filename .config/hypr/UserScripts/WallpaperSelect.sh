...
wallpaper_current="$HOME/.config/hypr/wallpaper_effects/.${focused_monitor}_wallpaper_current"
...
mapfile -d '' PICS < <(find -L "${wallDIR}" -type f \( \
  ...
  # Disabling video, because mpvpaper not working fine (not applying only to eDP-1, and avoiding the dGPU to sleep)
  ...
  for pic_path in "${sorted_options[@]}"; do
    ...
    # gif preview
      ...
      if [[ ! -f "$cache_gif_image" ]]; then
        ...
        magick "$pic_path" -resize 854x480 "$cache_gif_image"
      fi
    ...
    elif file --mime-type "$pic_path" | grep -qE '^.*: image/' ; then
    # image preview for big images
      cache_preview_image="$HOME/.cache/image_preview/${pic_name}"
      if [[ ! -f "$cache_preview_image" ]]; then
        mkdir -p "$HOME/.cache/image_preview"

        magick_options=(-resize 854x480)
        # for 16 bits PNG, we reduce the color depth
        if file "${pic_path}" | grep -q "16-bit" ; then
          magick_options+=(-depth 8)
        fi
        # For PNGs, reduce the number of colors
        if file --mime-type "$pic_path" | grep -q 'image/png'; then
          magick_options+=(-colors 64)
        fi
        # Reduce quality of JPG files
        if file --mime-type "$pic_path" | grep -q 'image/jpeg'; then
          magick_options+=(-quality 75)
        fi

        magick "$pic_path" "${magick_options[@]}" "$cache_preview_image" &
      fi
      printf "%s\x00icon\x1f%s\n" "$pic_name" "$cache_preview_image"
    else
...
apply_image_wallpaper() {
  ...
  echo swww img -o "$focused_monitor" "$image_path" $SWWW_PARAMS
